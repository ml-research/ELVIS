...
def all_combinations_of_wrapped_groups(fixed_props, is_positive, cluster_num, obj_quantities, qualifiers, pin):
    wrappers = {
        'proximity': [
        ...
        for k in range(1, n + 1):
            for wrapper_combo in itertools.combinations(wrapper_pairs, k):
                ...
                for group_id, (principle, wrapper) in enumerate(wrapper_combo, start=1):
                    grp_objs = wrapper(fixed_props, is_positive, cluster_num, obj_quantities, qualifiers, pin)
                    for obj in grp_objs:
