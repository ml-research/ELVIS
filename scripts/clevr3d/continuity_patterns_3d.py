"""
Continuity pattern task registration for 3D CLEVR.
Mirrors scripts/continuity/continuity_patterns.py.
"""

from scripts.utils.encode_utils import create_tasks_v3
from scripts.clevr3d.principles.continuity_3d import (
    non_overlap_two_splines_3d,
    non_overlap_non_intersected_splines_3d,
)
...
def get_patterns(lite=False):
    ...
    tasks, names = create_tasks_v3(
        non_overlap_two_splines_3d, feature_props, grp_num_range, size_list, pin
    )
    all_tasks.extend(tasks)
    all_names.extend(names)

    # Non-intersected splines
    tasks, names = create_tasks_v3(
        non_overlap_non_intersected_splines_3d, feature_props, grp_num_range, size_list, pin
    )
    all_tasks.extend(tasks)
    all_names.extend(names)
